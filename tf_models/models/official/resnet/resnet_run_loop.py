...
class AverageTrainMetrics(tf.train.SessionRunHook):
  ...
  def after_run(self, run_context, run_values):
    if self.step % self.log_freq == 0:
      ...
      format_str = ('%s: step %d, loss = %.3f, acc = %.4f, (%.1f examples/sec; %.3f '
                    'sec/batch)')
      if (mc.get_rank() == 0):
...
class AverageEvalMetrics(tf.train.SessionRunHook):
  ...
  def end(self,session):
    ...
    format_str = ('EVAL Session ENDED at %s: step %d, loss = %.3f, accuracy = %.4f (%.1f examples/sec; %.3f '
                  'sec/batch)')
    if (mc.get_rank() == 0):
...
def resnet_model_fn(features, labels, mode, model_class,
  ...
  if (mlcomm == 1):
      if mode == tf.estimator.ModeKeys.TRAIN: 
          train_hooks = [BcastTensors(), AverageTrainMetrics(loss,accuracy,log_freq,batch_size)]
          eval_hooks  = None
      else:
          train_hooks = None
...
def resnet_main(flags, model_function, input_function, num_train_samps, num_eval_samps, shape=None):
  ...
  if (flags.enable_ml_comm == 1):
      ...
      mc.config_team(0, 0, max_steps_train, max_steps_train, 0, 400)
  ...
  for _ in range(flags.train_epochs // flags.epochs_between_evals):
    ...
    esteps = num_eval_samps / flags.batch_size
